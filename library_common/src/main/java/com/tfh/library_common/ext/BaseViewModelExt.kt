...
import com.tfh.lib_base.ext.loge
import com.tfh.lib_base.network.BaseResponse
import com.tfh.lib_base.network.ResultState
import com.tfh.lib_base.network.exception.AppException
import com.tfh.lib_base.network.exception.ExceptionHandle
import com.tfh.lib_base.network.paresException
import com.tfh.lib_base.network.paresResult
...
fun <T> BaseViewModel.request(
    ...
    return viewModelScope.launch {
        runCatching {
            if (isShowDialog) commonChange.showDialog.postValue(loadingMessage)
            //请求体
            block()
        }.onSuccess {
            //网络请求成功 关闭弹窗
            commonChange.dismissDialog.postValue(false)
            runCatching {
                ...
                //打印错误消息
                e.message?.loge("onFailure---解析错误$e")
                //失败回调
            ...
            commonChange.dismissDialog.postValue(false)
            //打印错误消息
            it.message?.loge("onFailure---"+it.message)
            //失败回调
