...
object ActivityMessenger {
    ...
    inline fun startActivityForResult(
        ...
        startActivityForResult(
            starter,
            Intent(starter, target.java).putExtras(*params),
            callback
        )
    }
...
fun <T> extraFrag(extraName: String): FragmentExtras<T?> =
    FragmentExtras(extraName, null)

fun <T> extraFrag(extraName: String, defaultValue: T): FragmentExtras<T> =
    FragmentExtras(extraName, defaultValue)


fun <T> extraAct(extraName: String): ActivityExtras<T?> =
    ActivityExtras(extraName, null)
